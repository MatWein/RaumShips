...
import com.mw.raumships.client.PatchedMouseHelper;
...
public class PatchedEntityRenderer extends EntityRenderer implements IResourceManagerReloadListener {
    ...
    static PatchedMouseHelper patchedMouseHelper = new PatchedMouseHelper();
    ...
    public PatchedEntityRenderer(Minecraft mcIn, IResourceManager resourceManagerIn) {
    ...
    public void setThirdPersonDistance(float distance) {
        this.thirdPersonDistancePrev = this.thirdPersonDistance = distance;
    }
    ...
    public void updateCameraAndRender(float partialTicks, long nanoTime) {
        ...
        if (this.mc.inGameHasFocus && flag) {
            patchedMouseHelper.mouseXYChange();
            this.mc.getTutorial().handleMouse(patchedMouseHelper);
            float f = this.mc.gameSettings.mouseSensitivity * 0.6F + 0.2F;
            float f1 = f * f * f * 8.0F;
            float f2 = (float) patchedMouseHelper.deltaX * f1;
            float f3 = (float) patchedMouseHelper.deltaY * f1;
            int i = 1;
        ...
        if (!this.mc.skipRenderWorld) {
            ...
            if (this.mc.currentScreen != null) {
                ...
                try {
                    ...
                    crashreportcategory.addDetail("Screen name", () -> PatchedEntityRenderer.this.mc.currentScreen.getClass().getCanonicalName());
                    crashreportcategory.addDetail("Mouse location", () -> String.format("Scaled: (%d, %d). Absolute: (%d, %d)", k1, l1, Mouse.getX(), Mouse.getY()));
                    crashreportcategory.addDetail("Screen size", () -> String.format("Scaled: (%d, %d). Absolute: (%d, %d). Scale factor of %d", scaledresolution.getScaledWidth(), scaledresolution.getScaledHeight(), PatchedEntityRenderer.this.mc.displayWidth, PatchedEntityRenderer.this.mc.displayHeight, scaledresolution.getScaleFactor()));
