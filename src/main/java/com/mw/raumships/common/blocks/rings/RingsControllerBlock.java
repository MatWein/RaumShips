    ...
    public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
        ...
        if (heldItemStack.getItem() instanceof AnalyzerAncientItem) {
            if (!world.isRemote) {
                ...
                if (ringsTile != null) {
                    RaumShipsMod.proxy.getNetworkWrapper().sendTo(new StateUpdatePacketToClient(ringsTile.getPos(), ringsTile.getState(), ringsTile.getRings(), true), (EntityPlayerMP) player);
                }
            }
        } else {
            if (world.isRemote) {
